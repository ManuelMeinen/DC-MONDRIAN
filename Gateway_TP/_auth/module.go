...
func (m *Module) Handle(pkt chain.Packet) (chain.Packet, error) {
	if pkt.Ingress {
...
func (m *Module) handleIngress(pkt chain.Packet) (chain.Packet, error) {
	ts := time.Unix(int64(binary.LittleEndian.Uint32(pkt.RawPacket[timeOffset:timeOffset+timeLength])), 0)
	err := m.checkTime(ts)
	if err != nil {
		return chain.NilPacket, fmt.Errorf("[AuthIngress] verification failed: %v", err)
	}
	if pkt.RemoteTP == "" {
		return chain.NilPacket, fmt.Errorf("[AuthIngress] source TP address not set in packet")
	}
	zone, err := m.tr.GetZone(pkt.RawPacket)
	if err != nil {
		return chain.NilPacket, fmt.Errorf("%s %v", ingressPrefix, err)
	}
	key, err := m.km.DeriveL2Key(pkt.RemoteTP, zone)
	if err != nil {
		return chain.NilPacket, fmt.Errorf("[AuthIngress] key derivation failed: %v", err)
	}
...
func (m *Module) handleEgress(pkt chain.Packet) (chain.Packet, error) {
	if pkt.RemoteTP == "" {
		return chain.NilPacket, fmt.Errorf("[AuthEgress] destination TP address not set in packet")
	}
	key, fresh, err := m.km.FetchL2Key(pkt.RemoteTP, pkt.DstZone)
	if err != nil {
		return chain.NilPacket, fmt.Errorf("[AuthEgress] fetching L1 key for %v failed: %v", pkt.RemoteTP, err)
	}
	if fresh {
		m.tr.ResetState(pkt.RemoteTP)
		if err != nil {
			// how to handle? Next time we try fresh will be false and we don't set it here anymore
			return chain.NilPacket, err
		}
	}
	pkt.RawPacket, err = m.tr.ToIR(pkt.RemoteTP, key, pkt.RawPacket, pkt.DstZone)
	if err != nil {
		return chain.NilPacket, fmt.Errorf("[AuthEgress] proof creation failed: %v", err)
	}
	return pkt, nil
}
